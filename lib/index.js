...
class ElectronDownloader {
  ...
  get url () {
	const avoidMiddleUrl = !!this.opts.avoidMiddleUrl;

	console.log('&&&&&&&&&&& avoid mid url', avoidMiddleUrl)
	console.log('&&&&&&&&&&& baseurl', this.baseUrl )
	console.log('&&&&&&&&&&& middleurl', this.middleUrl )
	console.log('&&&&&&&&&&& slash', '/' )
	console.log('&&&&&&&&&&& suffix	', this.urlSuffix )

	// const fullUrl = path.normalize( `${this.baseUrl}${this.middleUrl}/${this.urlSuffix}`)
  ...
  downloadFile (url, cacheFilename, cb, onSuccess) {
    ...
    console.log('downloading', url, 'to', this.cache)
    const nuggetOpts = {
      target: tempFileName,
      dir: this.cache,
      resume: true,
